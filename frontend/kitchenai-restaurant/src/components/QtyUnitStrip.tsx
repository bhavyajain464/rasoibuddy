...
import { normalizeQtyOnInput, sanitizeQtyInput } from '../utils/qty';
...
export function QtyUnitStrip({
  ...
  const handleQtyChange = (text: string) => {
    const promoted = normalizeQtyOnInput(text, unit);
    if (promoted) {
      onUnitChange(promoted.unit);
      onQtyChange(promoted.qty);
      return;
    }
    onQtyChange(sanitizeQtyInput(text, unit));
  };
