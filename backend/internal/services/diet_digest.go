...
func (s *DietDigestService) BuildDigestBody(dateISO string, entries []CookedLogEntry, report *DietDayReport) (subject, plain, html string) {
	subject = fmt.Sprintf("Kitchen AI — diet report for %s", dateISO)
	if len(entries) == 0 {
		plain = fmt.Sprintf("You did not log any meals for %s.\n\nLog meals in Kitchen AI → Meals → History & Diet to receive a detailed PDF nutrition report.\n", dateISO)
		html = fmt.Sprintf("<p>You did not log any meals for <strong>%s</strong>.</p><p>Log meals in <strong>Kitchen AI → Meals → History &amp; Diet</strong> to receive your PDF nutrition report.</p>", dateISO)
...
// formatRFC5322From quotes display names that contain spaces (Gmail rejects bare "Kitchen AI <...>").
